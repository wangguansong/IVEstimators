IVEstPreAverage <- function(stock, bandwidth, type="logreturn",
  ...
  #     be computed.
  # Returns:
  #   A number, Pre-Averaging IV estimator
  ##################################################
  # Input Check
  ...
  pre.avg.logret <- filter(x=log.ret, method="convolution",
  ...
  # compute estimator
  if (adj.flag) {
    return(1 / (1 - psi1/2/bandwidth^2/psi2) *
           (1/bandwidth/psi2 * sum(pre.avg.logret^2) *
            N/(N-bandwidth+2) - 
            psi1/2/bandwidth^2/psi2 * sum(log.ret^2)))
  } else {
    return(1/bandwidth/psi2 * sum(pre.avg.logret^2) - 
           psi1/2/bandwidth^2/psi2 * sum(log.ret^2))
  }
}
